...
export function AppCard({ app }: { app: App }) {
    return (
        <Link href={`/apps/${app.packageName}`} className="group flex min-h-21 items-center gap-3 rounded-sm surface-vial p-3 transition-shadow hover:ring-1 hover:ring-elixir/60">
            {app.iconUrl ? (
